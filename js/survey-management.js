    ...
    sidebarToggle.addEventListener('click', toggleSidebar);
        ...
        async function loadSurveys() {
            // We now use the global state variable to log the current mode.
            console.log(`Attempting to load surveys... (Archived view: ${isShowingArchivedSurveys})`);
            
            // We build the API URL dynamically based on the toggle's state.
            const apiUrl = isShowingArchivedSurveys 
                ? 'api/surveys.php?show_archived=true' 
                : 'api/surveys.php';

            try {
                // We use the new 'apiUrl' variable in the fetch call.
                const response = await fetch(apiUrl);
            ...
            // The render call remains at the end, which is perfect.
            renderSurveys();
        }
        ...
        function renderSurveys() {
            ...
            // 1. Handle the "empty state" for both active and archived views.
            ...
            // 2. If there are surveys, build the table rows.
        ...
        async function reactivateSurvey(surveyId) {
            try {
                // Use a non-destructive confirmation modal
                await showConfirmationModal({
                    title: 'Reactivate Survey',
                    message: 'Are you sure you want to reactivate this survey? It will appear in the main list again.',
                    actionText: 'Yes, Reactivate',
                    destructive: false
                });

                const response = await fetch(`api/surveys.php?action=reactivate&id=${surveyId}`, {
                    method: 'PUT',
        ...
        async function permanentlyDeleteSurvey(surveyId) {
            try {
                // This confirmation MUST be very clear about the danger
                await showConfirmationModal({
                ...
                // Use a different API endpoint for this dangerous action
                const response = await fetch(`api/permanent-delete-survey.php?id=${surveyId}`, {
                    method: 'DELETE'
                });
                const result = await response.json();
                if (result.success) {
        ...
        function getSurveyLink(surveyId) {
            ...
            // window.location.origin gives you the base URL
        ...
        async function deleteSurvey(surveyId) {
            try {
                ...
                await showConfirmationModal({
                    ...
                    message: 'Are you sure you want to archive this survey? It will be hidden from the list and will no longer accept new responses.',
                    actionText: 'Yes, Archive',
                    destructive: true //Make button red
                });

                //if the user confirms, we proceed with the archival
